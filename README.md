...
- 字幕匹配：每个条目右上角新增圆形 `×` 删除按钮，删除前会弹出二次确认。
- 字幕匹配删除：单条删除为硬删除，会删除视频文件本身，并尝试一并删除 `sub` 目录下同名字幕文件。
- 字幕匹配批量：顶部 `批量` 菜单新增 `批量删除`，输入集数并确认后，会按匹配规则识别集数，硬删除所有集数小于等于该值的条目。
- 测试：补充批量删除集数阈值规则的单元测试。
...
- 单条操作按钮：`匹配`、`偏移`、`播放`、右上角圆形 `×` 删除按钮
- 删除行为：删除前二次确认；确认后硬删除视频文件，并删除 `sub` 目录下同名字幕文件
