...
- 字幕匹配：修复“批量匹配完成后条目仍显示匹配中”的问题。批量任务结束后会正确回写每条视频的最终状态，并关闭匹配中的 UI 标记。
- 种子下载：列表页新增 `Push` / `Pull` 按钮，用于同步订阅配置（URL 列表）。
- 远端仓库：在仓库根目录使用 `seed-subscriptions.json` 保存订阅条目，仅包含 URL 信息。
- 同步约束：`seed-subscriptions.json` 不包含设备信息、不绑定本地文件夹、不记录下载状态。
- 页面展示：该同步文件仅用于种子下载页解析与展示，不会出现在远程同步页面中。
- 字幕匹配交互：当顶部按钮显示 `批量中...` 时可点击，点击后会弹出确认框，确认可中断当前批量匹配/批量偏移任务。
- 种子下载交互：删除订阅条目前增加二次确认弹窗，确认后才会执行删除。
